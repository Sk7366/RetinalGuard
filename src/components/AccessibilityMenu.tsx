...
export const AccessibilityMenu: React.FC<AccessibilityMenuProps> = ({
  ...
  const handleColorVisionChange = (cv: ColorVisionOption) => {
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-88 bg-white dark:bg-[#1B161A] rounded-2xl border border-stone-200 dark:border-[#33292F] shadow-xl p-4 z-50 animate-in fade-in zoom-in-95 duration-150 text-[#2B2024] dark:text-[#FAF5F7] max-h-[85vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {[
                ...
                <button
                  key={cv.id}
                  type="button"
                  onClick={() => handleColorVisionChange(cv.id as ColorVisionOption)}
          ...
          {/* 4. High Contrast & Reduce Motion Toggles */}
          ...
          {/* 6. Open Full Centralized Accessibility Settings Modal */}
          {onOpenFullSettings && (
            <button
              type="button"
              onClick={() => {
