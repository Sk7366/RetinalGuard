    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-16">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-[#6F6267] mt-1 max-w-3xl leading-relaxed">
              ...
              <strong className="text-[#F05A28]">+0.068 AUC increase (p &lt; 0.001)</strong> over baseline fundus-only classification.
            </p>
          </div>

          <div className="flex items-center gap-2 text-xs font-mono bg-[#FFFDF9] p-3 rounded-xl border border-[#EFE4DC]">
            <div>
              <span className="text-[#9E8D91] block text-[10px]">Statistical Test:</span>
              <span className="font-bold text-[#F05A28]">DeLong p &lt; 0.001</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#EFE4DC] pb-4">
          <div>
            <h2 className="text-lg font-serif font-bold text-[#2B2024] flex items-center gap-2">
              <BarChart2 className="w-5 h-5 text-[#F05A28]" />
          ...
          <div className="flex items-center gap-1 bg-[#FFFDF9] p-1 rounded-xl border border-[#EFE4DC] text-xs">
            <button
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-colors ${
                selectedMetric === 'sensitivitySeverePdr'
                  ? 'bg-[#D84818] text-white shadow-xs'
                  : 'text-[#6F6267] hover:text-[#2B2024]'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1 rounded-lg font-semibold transition-colors ${
                selectedMetric === 'latencyMs'
                  ? 'bg-[#D94A78] text-white shadow-xs'
                  : 'text-[#6F6267] hover:text-[#2B2024]'
              }`}
            >
              Latency (ms)
            ...
            return (
              <div
                ...
                className={`p-4 rounded-xl border transition-all ${
                  isFull
                    ? 'bg-gradient-to-r from-[#FFE5D8] to-[#FBE4EC] border-[#FDBA74] shadow-xs'
                    : 'bg-white border-[#EFE4DC] hover:bg-[#FFFDF9]'
                }`}
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <strong className="text-[#2B2024] text-sm">{exp.name}</strong>
                    {isFull && (
                      <span className="px-2 py-0.5 rounded-full bg-gradient-to-r from-[#F05A28] to-[#DB2777] text-white font-bold text-[10px]">
                        RetinaGuard Tri-Modal
                      </span>
                    )}
                    {isFundusBaseline && (
                      <span className="px-2 py-0.5 rounded-full bg-[#EFE4DC] text-[#6F6267] font-medium text-[10px]">
                        Baseline Reference
                      </span>
                    )}
                  </div>
                ...
                <div className="h-3 w-full bg-[#EFE4DC]/60 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-700 ${
                      isFull
                        ? 'bg-gradient-to-r from-[#F05A28] to-[#DB2777]'
                        : isFundusBaseline
                        ? 'bg-[#FED7AA]'
                        : 'bg-[#FDBA74]'
                    }`}
                ...
                <p className="text-[11px] text-[#6F6267] mt-2 italic">
      ...
      <section className="bg-white rounded-2xl border border-[#EFE4DC] p-6 sm:p-8 space-y-6">
        <div className="border-b border-[#EFE4DC] pb-4">
          <h2 className="text-xl font-serif font-bold text-[#2B2024]">Complete 6-Experiment Ablation Matrix</h2>
                ...
                return (
                  <tr
                    ...
                    className={`transition-colors ${
                      isWinner ? 'bg-[#FFE5D8] font-semibold' : 'hover:bg-[#FFFDF9]'
                    }`}
                  >
                    <td className="py-3.5 px-4 text-[#2B2024]">
                      <span className="font-bold">{exp.name}</span>
                    </td>
                    <td className="py-3.5 px-4 font-mono text-[#6F6267]">
                      {exp.modalities}
                    </td>
                    <td className="py-3.5 px-3 text-center font-bold text-[#F05A28]">
                      {exp.auc.toFixed(3)}
                    </td>
