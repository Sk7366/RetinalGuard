...
export const PatientReportsView: React.FC<PatientReportsViewProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 text-[#2B2024]">
      {/* HEADER */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-[#EFE4DC]">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-[#FFE5D8] text-[#D84818] border border-[#FED7AA] mb-1.5">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#F05A28] hover:bg-[#D84818] text-white text-xs font-semibold shadow-xs transition-colors self-start sm:self-auto cursor-pointer"
        >
      ...
      <div className="p-4 rounded-2xl bg-[#FFE5D8] dark:bg-[#2A1D17] border border-[#FED7AA] dark:border-[#F05A28]/30 text-xs sm:text-sm text-[#7C2D12] dark:text-[#FDBA74] flex items-start gap-3">
      ...
      <div className="p-3.5 rounded-2xl bg-[#FFFDF9] border border-[#EFE4DC] flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
        <div className="flex items-center gap-2.5">
          <Mail className="w-4 h-4 text-[#F05A28]" />
          <div>
            <span className="font-semibold text-[#2B2024]">{t("deliveryEmailLabel", "Delivery Email:")} </span>
            <span className="text-[#6F6267] font-mono">{currentUser.email}</span>
        ...
        {!currentUser.emailVerified && (
          <button
            type="button"
            onClick={onOpenAuth}
            className="text-xs font-semibold text-[#F05A28] hover:underline shrink-0"
          >
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                <div className="flex items-center gap-2 text-xs text-[#9E8D91]">
                  ...
                  <span className="font-semibold text-[#2B2024]">{report.date}</span>
                  <span>•</span>
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-[#EFE4DC] bg-white hover:bg-[#FFFDF9] text-xs font-semibold text-[#2B2024] transition-colors"
                  >
                    <Send className="w-3.5 h-3.5 text-[#F05A28]" />
                    <span>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-xs text-[#6F6267] hover:text-[#2B2024] font-medium px-2 py-1"
                  >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-[#2B2024] hover:bg-black text-white text-xs font-semibold transition-colors"
                  >
              ...
              {isTechExpanded && report.technicalDetails && (
                <div className="p-4 rounded-xl bg-[#FFFDF9] border border-[#EFE4DC] space-y-3 text-xs animate-in fade-in">
                  <div className="flex items-center justify-between font-bold text-[#2B2024] pb-1 border-b border-[#EFE4DC]">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[#6F6267]">
                    <div>
                      <span className="font-semibold text-[#2B2024] block">{t("inferenceModelLabel", "Inference Model:")}</span>
                      <span>{report.technicalDetails.model}</span>
                    </div>
                    <div>
                      <span className="font-semibold text-[#2B2024] block">{t("modalitiesAnalyzedLabel", "Modalities Analyzed:")}</span>
                      <span>{report.technicalDetails.modalities}</span>
                    </div>
                    <div>
                      <span className="font-semibold text-[#2B2024] block">{t("modelConfidenceLabel", "Model Confidence:")}</span>
                      <span className="font-mono font-bold text-[#F05A28]">{report.technicalDetails.confidence}</span>
                    </div>
                    <div>
                      <span className="font-semibold text-[#2B2024] block">{t("gradCamSalienceFocusLabel", "Grad-CAM Salience Focus:")}</span>
                      <span>{report.technicalDetails.gradCamRegion}</span>
                    </div>
                  </div>

                  <div>
                    <span className="font-semibold text-[#2B2024] block mb-1">{t("keyContributingShapLabel", "Key Contributing SHAP Factors:")}</span>
                    <ul className="list-disc list-inside text-[#6F6267] space-y-0.5">
