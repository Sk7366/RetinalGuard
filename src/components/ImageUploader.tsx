...
export const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  return (
    <div className="space-y-4" id={`${idPrefix}-root`}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-bold text-[#2B2024]">{label}</h3>
            {required ? (
              <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-[#FEE2E2] text-[#DC2626]">
                {t('requiredModality', 'Required')}
              </span>
            ) : (
              <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-[#F5F1ED] text-[#6F6267]">
                {t('optionalModality', 'Optional')}
              </span>
            )}
          </div>
          {sublabel && <p className="text-xs text-[#6F6267] mt-0.5">{sublabel}</p>}
        </div>

        {/* Quick Sample Presets (if available) */}
        {presets.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[11px] font-medium text-[#6F6267] mr-1">{t('sampleScansLabel', 'Sample Scans:')}</span>
            {presets.map((preset) => {
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors border ${
                    isSelected
                      ...
                      : 'bg-[#FFFDF9] text-[#2B2024] border-[#EFE4DC] hover:border-[#F05A28] hover:bg-[#FFE5D8]'
                  }`}
        ...
        <div className="rounded-2xl border border-[#EFE4DC] bg-[#181517] overflow-hidden shadow-xs space-y-0">
          ...
          <div className="px-4 py-2.5 bg-[#231F21] border-b border-[#3B3437] flex items-center justify-between text-xs text-white">
            <div className="flex items-center gap-2 truncate pr-2">
              <FileImage className="w-4 h-4 text-[#F05A28] shrink-0" />
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1 rounded text-[11px] font-medium transition-colors ${
                  showingClahe
                    ...
                    : 'bg-[#2B2024] text-[#D1D5E0] hover:text-white border border-[#483F43]'
                }`}
              >
          ...
          <div className="p-3 bg-[#FAF8F6] border-t border-[#EFE4DC] flex flex-wrap items-center justify-between gap-2">
            <div className="text-[11px] text-[#6F6267] flex items-center gap-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl bg-white border border-[#EFE4DC] text-[#2B2024] hover:bg-[#FFE5D8] hover:border-[#F05A28] hover:text-[#F05A28] transition-colors shadow-2xs"
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-xl bg-white border border-[#EFE4DC] text-[#2B2024] hover:bg-[#FAF8F6] transition-colors shadow-2xs"
        ...
        <div
          ...
          className={`cursor-pointer rounded-2xl border-2 border-dashed p-8 text-center transition-all ${
            isDragging
              ...
              : 'border-[#E5D7CE] bg-[#FAF8F6] hover:border-[#F05A28] hover:bg-[#FFFDF9]'
          }`}
        >
          <div className="mx-auto w-12 h-12 rounded-2xl bg-white border border-[#EFE4DC] flex items-center justify-center text-[#F05A28] shadow-2xs mb-3">
          ...
          <div className="space-y-1">
            <div className="text-sm font-bold text-[#2B2024]">
              ...
              <span className="text-[#F05A28] underline decoration-[#F05A28]/40">{t('browseFile', 'browse file')}</span>
            </div>
          ...
          {helperHint && (
            <div className="mt-4 inline-flex items-center gap-1.5 text-[11px] text-[#6F6267] bg-white px-3 py-1 rounded-full border border-[#EFE4DC]">
      ...
      {showRetakeGuide && (
        <div className="p-4 rounded-2xl bg-[#FFFDF9] border border-[#FDBA74] shadow-xs space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="p-1 rounded-lg text-[#6F6267] hover:bg-[#F5F1ED]"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <ul className="space-y-1.5 text-xs text-[#6F6267]">
            {(retakeInstructions.length > 0
              ...
              <li key={idx} className="flex items-start gap-2">
                <span className="w-4 h-4 rounded-full bg-[#FFEDD5] text-[#F05A28] font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">
                  {idx + 1}
                </span>
          ...
          <div className="pt-2 flex items-center justify-end gap-2 border-t border-[#EFE4DC]">
            <button
              type="button"
              onClick={() => fileInputRef.current?.click()}
              className="px-3 py-1.5 rounded-xl bg-[#F05A28] text-white text-xs font-bold hover:bg-[#D84818] transition-colors shadow-2xs"
            >
